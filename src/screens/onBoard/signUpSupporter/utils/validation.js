...
export const handleValidation = async (formData, key, value, getVal) => {
  ...
  switch (key) {
    ...
    case 'occupation':
      if (!value) {
        error = getVal('occupationRequired') || 'occupation is required';
      }
      break;
    case 'company':
      if (!value) {
        error = getVal('companyRequired') || 'company is required';
      }
      break;
    case 'behalfCompany':
      if (!value) {
        error = getVal('behalfCompanyRequired');
      }
      break;
    case 'wantsAnonymous':
      if (!value) {
        error = getVal('wantsAnonymousRequired');
      }
      break;
    default:
      break;
